...
function usernameDisplay(){
	...
	if(notLogin){
		...
		$display_login = <<<DELIMETER
			<ul class="nav navbar-nav navbar-right">
                    <li><a href="register.php"><span class="glyphicon glyphicon-info-sign"></span>Sign Up</a></li>
                    <li><a href="login.php"><span class="glyphicon glyphicon-user"></span> <span>Login</span></a></li>
...
function login_user(){
	if(isset($_POST['password'])){
		...
		$query = query("SELECT user_type, id FROM User INNER JOIN UserLogin ON User.id =UserLogin.user_id WHERE userlogin.username = '$username' ");
		confirm($query);
		
			if(mysqli_num_rows($query) == 0) {
				set_message("Your username is wrong.");
				redirect("login.php");
			}
			else if(password_verify($password,userlogin.password == false)){
				et_message("Your password is wrong.");
				redirect("login.php");
			}
        ...
        if(mysqli_num_rows($query_1) >= 1 ){
        	...
        	redirect("register.php");
					...
					{  
						...
						if (mysqli_num_rows($sql_3) == 0) { // insert new zip
